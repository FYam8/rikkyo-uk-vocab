...
function validBundle(): RuntimeBundle {
  ...
  return { release: { appId: APP_ID, productVersion: PRODUCT_VERSION, engineVersion: ENGINE_VERSION, datasetVersion: DATASET_VERSION, persistenceSchemaVersion: PERSISTENCE_SCHEMA_VERSION, exportFormatVersion: EXPORT_FORMAT_VERSION, indexedDbVersion: INDEXED_DB_VERSION, commonEngineCommit: "test", enrichmentVersion: ENRICHMENT_VERSION }, manifest: { appId: APP_ID, dataVersion: DATASET_VERSION, registryEntityCount: REGISTRY_ENTITY_COUNT, coreEntityCount: CORE_ENTITY_COUNT, generatedFromPhase: 24, enrichmentVersion: ENRICHMENT_VERSION, sourcePapers: ["FY24-A","FY24-B","FY25-A","FY25-B","FY26-A","FY26-B"] }, registry, core };
}
...
describe("runtime data gate", () => {
  it("bypasses an older service-worker cache with a release-qualified URL", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith("/rikkyo-uk-vocab/data/manifest.json?appRelease=3.6.1", { cache: "no-store" });
  });
  ...
  it("requires the Phase 24 reselection marker", () => { const b = validBundle(); (b.manifest as { generatedFromPhase: number }).generatedFromPhase = 17; expect(validateRuntimeBundle(b).ok).toBe(false); });
  it("requires 912 registry metadata", () => { const b = validBundle(); b.manifest.registryEntityCount = 1; expect(validateRuntimeBundle(b).ok).toBe(false); });
  ...
  it("requires 912 registry records", () => { const b = validBundle(); b.registry.pop(); expect(validateRuntimeBundle(b).ok).toBe(false); });
  ...
  it("passes a complete Phase 24 fixture", () => expect(validateRuntimeBundle(validBundle()).ok).toBe(true));
