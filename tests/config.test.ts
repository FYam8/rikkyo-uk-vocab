...
describe("application isolation", () => {
  ...
  it("keeps v3 persistence as the compatibility origin across product releases", () => { expect(PRODUCT_VERSION).toBe("3.5.6"); expect(PERSISTENCE_SCHEMA_VERSION).toBe(3); expect(INDEXED_DB_VERSION).toBe(3); });
