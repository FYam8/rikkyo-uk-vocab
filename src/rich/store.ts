...
export async function ensureGeneration(db: IDBDatabase, dataVersion: string, registryCount: number, coreCount: number): Promise<GenerationMeta> {
  ...
  const prefs: Preferences = { key: "preferences", generationId: generation.generationId, dailyTargetSeconds: DAY_TARGET, learningTimeZone: "Europe/London", examDate: null, diagnosticCompleted: false, studyMode: "recommended", sessionSize: 20, scheduleFilter: "all", accent: "auto", voiceURI: "", theme: "auto", audioQuestions: "auto", lastAppliedRevision: revision };
...
export async function savePreferences(db: IDBDatabase, writer: SingleWriter, patch: Partial<Pick<Preferences, "dailyTargetSeconds" | "learningTimeZone" | "examDate" | "diagnosticCompleted" | "studyMode" | "sessionSize" | "scheduleFilter" | "accent" | "voiceURI" | "theme" | "audioQuestions">>): Promise<void> {
